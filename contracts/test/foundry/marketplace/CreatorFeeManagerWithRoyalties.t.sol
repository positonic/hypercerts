...
contract CreatorFeeManagerWithRoyaltiesTest is ProtocolBase {
  ...
  function _setUpRoyaltiesRegistry(uint256 fee) internal {
    vm.prank(_owner);
    royaltyFeeRegistry.updateRoyaltyInfoForCollection(address(mockERC721), _royaltyRecipient, _royaltyRecipient, fee);
  }
  ...
  function testCreatorRoyaltiesGetPaidForRoyaltyFeeManagerWithBundles() public {
    ...
    (OrderStructs.Maker memory makerBid, OrderStructs.Taker memory takerAsk) = _createMockMakerBidAndTakerAskWithBundle(
      address(mockERC721),
      address(weth),
      numberItemsInBundle
    );
  ...
  function testCreatorRoyaltiesRevertForEIP2981WithBundlesIfInfoDiffer() public {
    ...
    vm.prank(takerUser);
    vm.expectRevert(
      abi.encodeWithSelector(ICreatorFeeManager.BundleEIP2981NotAllowed.selector, address(mockERC721WithRoyalties))
    );
