...
// Shared errors
import { AmountInvalid, OrderInvalid, FunctionSelectorInvalid, MerkleProofInvalid, QuoteTypeInvalid, CollectionTypeInvalid } from "@hypercerts/marketplace/errors/SharedErrors.sol";
import { MAKER_ORDER_PERMANENTLY_INVALID_NON_STANDARD_SALE, ORDER_EXPECTED_TO_BE_VALID } from "@hypercerts/marketplace/constants/ValidationCodeConstants.sol";

// Strategies
import { StrategyHypercertCollectionOffer } from "@hypercerts/marketplace/executionStrategies/StrategyHypercertCollectionOffer.sol";
...
contract HypercertCollectionOffersTest is ProtocolBase {
  ...
  function testMakerBidAmountsLengthNotOne() public {
    ...
    _assertMakerOrderReturnValidationCode(makerBid, signatureMaker, MAKER_ORDER_PERMANENTLY_INVALID_NON_STANDARD_SALE);
  ...
  function testTakerAskCannotExecuteWithInvalidProof(uint256 itemIdSold) public {
    ...
    OrderStructs.Taker memory takerAsk = OrderStructs.Taker(
      takerUser,
      abi.encode(((itemIdInMerkleTree << 128) + 1), 10_000, proof)
    );
  ...
  function testTakerAskCannotExecuteSignatureForDifferentToken(uint256 itemIdSold) public {
    ...
    OrderStructs.Taker memory takerAsk = OrderStructs.Taker(
      takerUser,
      abi.encode(((itemIdInMerkleTree << 128) + 2), 10_000, proof)
    );
  ...
  function testTakerAskHypercertCollectionOrderWithMerkleTreeAccountAllowlist() public {
    ...
    bytes memory signatureTaker = _signTakerDataCollectionStrategy(
      makerBid,
      ((tokenIdInMerkleTree << 128) + 1),
      proof,
      takerUserPK
    );
    ...
    OrderStructs.Taker memory takerAsk = OrderStructs.Taker(
      takerUser,
      abi.encode(((tokenIdInMerkleTree << 128) + 1), 10_000, proof, signatureTaker)
    );
  ...
  function testTakerAskCannotExecuteWithInvalidProofAccountAllowlist(uint256 itemIdSold) public {
    ...
    OrderStructs.Taker memory takerAsk = OrderStructs.Taker(
      takerUser,
      abi.encode(((1 << 128) + 1), 10_000, proof, signatureTaker)
    );
  ...
  function testInvalidSelector() public {
    ...
    (bool orderIsValid, bytes4 errorSelector) = strategyHypercertCollectionOffer.isMakerOrderValid(makerBid, bytes4(0));
  ...
  function testWrongQuoteType() public {
    ...
    (bool orderIsValid, bytes4 errorSelector) = strategyHypercertCollectionOffer.isMakerOrderValid(
      makerAsk,
      selectorNoProof
    );
