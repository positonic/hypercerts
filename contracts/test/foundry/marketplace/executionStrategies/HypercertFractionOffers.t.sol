...
// Shared errors
import { AmountInvalid, LengthsInvalid, OrderInvalid, FunctionSelectorInvalid, MerkleProofInvalid, QuoteTypeInvalid } from "@hypercerts/marketplace/errors/SharedErrors.sol";
...
// Strategies
import { StrategyHypercertFractionOffer } from "@hypercerts/marketplace/executionStrategies/StrategyHypercertFractionOffer.sol";
...
contract HypercertFractionOrdersTest is ProtocolBase {
  ...
  function _createMakerAskAndTakerBidHypercert(
    bool mint
  ...
  function testmakerAskAmountsLengthNotOne() public {
    ...
    (OrderStructs.Maker memory makerAsk, OrderStructs.Taker memory takerBid) = _createMakerAskAndTakerBidHypercert(
      true
    );
  ...
  function testMakerAskInvalidation() public {
    ...
    vm.prank(makerUser);
    mockHypercertMinter.mintClaimWithFractions(makerUser, 10_000, fractions, "https://example.com", FROM_CREATOR_ONLY);
  ...
  function testTakerBidHypercertFractionOrderWithMerkleTreeHypercertAccountAllowlist() public {
    ...
    makerAsk.strategyId = 2;
    makerAsk.additionalParameters = abi.encode(minUnitAmount, maxUnitAmount, minUnitsToKeep, sellLeftover, merkleRoot);
  ...
  function testInvalidSelector() public {
    ...
    (bool orderIsValid, bytes4 errorSelector) = strategyHypercertFractionOffer.isMakerOrderValid(makerAsk, bytes4(0));
  ...
  function _assertOrderIsValid(OrderStructs.Maker memory makerAsk, bool withProof) private {
    (bool orderIsValid, bytes4 errorSelector) = strategyHypercertFractionOffer.isMakerOrderValid(
      makerAsk,
      withProof ? selectorWithProofAllowlist : selectorNoProof
    );
  ...
  function _assertSuccessfulTakerBid(
    ...
    assertEq(
      ...
      _initialWETHBalanceUser +
        (unitAmount * bidPrice * _sellerProceedBpWithStandardProtocolFeeBp) /
