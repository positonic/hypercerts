  ...
  OwnableTwoSteps
{
   ...
   * @param to Recipient address
   */
  function transferBatchItemsAcrossCollections(BatchTransferItem[] calldata items, address from, address to) external {
  ...
  function _processBatchHypercertCollection(BatchTransferItem calldata batchItems, address from, address to) private {
    uint256[] calldata itemIds = batchItems.itemIds;
    uint256[] calldata amounts = batchItems.amounts;
    uint256 length = itemIds.length;
