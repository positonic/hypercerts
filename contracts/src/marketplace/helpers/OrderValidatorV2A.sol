...
contract OrderValidatorV2A {
  ...
  function _checkValidityCurrencyAndStrategy(
    ...
    if (strategyId != 0) {
      if ((strategyIsMakerBid && quoteType != QuoteType.Bid) || (!strategyIsMakerBid && quoteType != QuoteType.Ask)) {
   ...
   * @return validationCode Validation code
   */
  function _checkValidityTimestamps(uint256 startTime, uint256 endTime) private view returns (uint256 validationCode) {
  ...
  function _checkValidityERC721AndEquivalents(
    ...
    (success, data) = collection.staticcall(abi.encodeCall(IERC721.isApprovedForAll, (user, address(transferManager))));
  ...
  function _checkValidityMakerAskItemIdsAndAmountsAndPrice(
    ...
    if (makerAsk.strategyId == 0) {
      ...
      validationCode = _getOrderValidationCodeForStandardStrategy(makerAsk.collectionType, itemIds.length, amounts);
    } else {
      itemIds = makerAsk.itemIds;
      amounts = makerAsk.amounts;
  ...
  function _checkValidityMakerBidItemIdsAndAmountsAndPrice(
    ...
    if (makerBid.strategyId == 0) {
      ...
      validationCode = _getOrderValidationCodeForStandardStrategy(makerBid.collectionType, itemIds.length, amounts);
    } else {
      // @dev It should ideally be adjusted by real price
