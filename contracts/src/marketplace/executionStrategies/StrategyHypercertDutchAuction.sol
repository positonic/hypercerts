...
// Shared errors
import { AmountInvalid, BidTooLow, OrderInvalid, FunctionSelectorInvalid, QuoteTypeInvalid, CollectionTypeInvalid } from "../errors/SharedErrors.sol";
...
contract StrategyHypercertDutchAuction is BaseStrategy {
  ...
  function executeStrategyWithTakerBid(
    ...
    (uint256 startPrice, uint256[] memory unitsPerItem) = abi.decode(
  ...
  function isMakerOrderValid(
    ...
    if (
      itemIdsLength == 0 ||
      unitsPerItem.length == 0 ||
      unitsPerItem.length != itemIdsLength ||
