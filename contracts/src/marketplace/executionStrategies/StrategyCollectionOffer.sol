...
// Shared errors
import { OrderInvalid, FunctionSelectorInvalid, MerkleProofInvalid, QuoteTypeInvalid, CollectionTypeInvalid } from "../errors/SharedErrors.sol";
...
contract StrategyCollectionOffer is BaseStrategy {
    ...
    returns (uint256 price, uint256[] memory itemIds, uint256[] calldata amounts, bool isNonceInvalidated)
  {
    ...
    (uint256 offeredItemId, bytes32[] memory proof) = abi.decode(takerAsk.additionalParameters, (uint256, bytes32[]));
    itemIds = new uint256[](1);
    itemIds[0] = offeredItemId;
    isNonceInvalidated = true;
