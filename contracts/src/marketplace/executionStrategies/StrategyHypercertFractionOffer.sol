...
// Shared errors
import { AmountInvalid, LengthsInvalid, OrderInvalid, FunctionSelectorInvalid, MerkleProofInvalid, QuoteTypeInvalid } from "../errors/SharedErrors.sol";
...
contract StrategyHypercertFractionOffer is BaseStrategy {
  ...
  function executeHypercertFractionStrategyWithTakerBid(
  ...
  ) external view returns (uint256 price, uint256[] memory itemIds, uint256[] memory amounts, bool isNonceInvalidated) {
    ...
    (uint256 minUnitAmount, uint256 maxUnitAmount, uint256 minUnitsToKeep, bool sellLeftover) = abi.decode(
  ...
  function executeHypercertFractionStrategyWithTakerBidWithAllowlist(
    ...
    (uint256 unitAmount, uint256 pricePerUnit, bytes32[] memory proof) = abi.decode(
      takerBid.additionalParameters,
      (uint256, uint256, bytes32[])
    );

    //minUnitAmount, maxUnitAmount, minUnitsToKeep, sellLeftover, root
    (uint256 minUnitAmount, uint256 maxUnitAmount, uint256 minUnitsToKeep, bool sellLeftover, bytes32 root) = abi
      .decode(makerAsk.additionalParameters, (uint256, uint256, uint256, bool, bytes32));

    // A collection order can only be executable for 1 itemId but quantity to fill can vary
    if (sellLeftover) {
      // Allow for selling a fraction holding units lower
      if (
        minUnitAmount > maxUnitAmount ||
        unitAmount == 0 ||
  ...
  function isMakerOrderValid(
    ...
    (uint256 minUnitAmount, uint256 maxUnitAmount, uint256 minUnitsToKeep) = abi.decode(
      makerAsk.additionalParameters,
