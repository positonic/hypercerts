...
contract ExecutionManager is InheritedStrategy, NonceManager, StrategyManager, IExecutionManager {
    ...
    )
  {
    ...
    (recipients[1], feeAmounts[1]) = _getCreatorRecipientAndCalculateFeeAmount(makerOrder.collection, price, itemIds);
    if (makerOrder.quoteType == QuoteType.Bid) {
      ...
      _setTheRestOfFeeAmountsAndRecipients(makerOrder.strategyId, price, makerOrder.signer, feeAmounts, recipients);
    }
  }
